...
public class KnapsackHelper {
    ...
    public static Settings clientSettings(Client client, Environment environment, KnapsackRequest request) {
        ...
        if (host == null) {
            ...
            if (cluster == null) {
                cluster = response.getClusterName();
            }
        }
        if (cluster == null) {
            NodesInfoResponse response = client.admin().cluster()
        ...
        return settingsBuilder()
                ...
                .put("timeout", request.getTimeout().millis()) // our timeout for instantiating TransportClient
                .put("client.transport.sniff", request.getSniff()) // sniff = look for other nodes
                .put("client.transport.ping_timeout", request.getTimeout().millis()) // timeout for the transport connection
